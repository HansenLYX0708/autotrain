...
| `interfaces` | 2, 3 | `a1/a2/b1/b2/b3/b4`、`a1→b1`/`a1→b3`/`a2→b2`/`a2→b4` 的 dx/dy、5 nm 内最大向下偏差 |
| `mgo_c` | 4 | `MgO_C` 拟合线段（端点即 `b3`/`b4`）、fit R²、角度 |
...
`b1`/`b2` = `MgO_C` **骨架**的 x 最小/最大端（原始端点），
`b3`/`b4` = `MgO_C` **拟合线段**的两个端点（把上述极值点投影到拟合直线上）。
`b1` 与 `b3` 的差别就是该处层的局部起伏，所以 `a1→b1` 与 `a1→b3` 同时给出：前者对准真实
端点，后者对准直线模型、对局部毛刺不敏感。实测两者 dx 几乎一致（中位数 5.332 / 5.333
nm），dy 的离散度 `b3` 更小。偏差符号：`dx>0` 表示 b 在 a 右侧，
...
`Non_mag1`/`Non_mag2` 是把这条拟合直线**向外延伸到 `Non_mag` 区域的 x 最小/最大列**所得的
交点，而不是停在参与拟合的列上。被 σ 剔除的恰恰是左右两端圆角掉的列，停在那里会让端点
随剔除阈值内缩；延伸到区域边界后，端点是区域的属性而不是阈值的属性。y 取拟合直线的值
（而非原始像素），所以圆角本身不会污染纵坐标。`max_extrapolation` 记录了延伸了多少。
...
| 参数 | 默认 | 说明 |
|------|------|------|
| `--scale-nm` | `auto` | `auto` 从文件名读视场 ÷ 图像宽度；也可直接给 nm/px 数值 |
| `--fov-nm` | — | 直接指定视场(nm)，仍然除以图像宽度 |
...
**标尺从文件名读。** 默认 `--scale-nm auto`：用正则从文件名里取视场
（`..._FOV_76.37nm.png` → 76.37 nm），再除以**图像实际宽度**得到 nm/px。样本批次
26 张全部解析成功，得到 `76.37 / 1024 = 0.074580 nm/px`。

除以实际宽度而不是写死 1024，是为了掩膜万一以别的尺寸导出时仍然正确；宽度不是 1024 时
会 warning 提示。`--fov-nm` 可直接给视场，`--scale-nm <数值>` 可完全绕过解析。
JSON/CSV 里都记了 `fov_nm` / `image_width_px` / `scale_nm_per_px` / `scale_source`，
事后可复核或换算。解析不到视场且没给 `--scale-nm` 时会直接报错，不会静默用错标尺。
...
- 标尺：文件名解析（含无视场时返回 `None`）；端到端跑一张 `..._FOV_76.37nm.png`，
  `nm/px` 必须精确等于 `76.37 / 图像宽度`，且任一 nm 值精确等于其 px 值乘以该标尺
- `b3`/`b4`：合成图的 `MgO_C` 是完美矩形，拟合端点必须与骨架端点重合，
  `a1→b3` 与 `a1→b1` 必须相同
