...
export function DatasetsPage() {
  ...
  const generateYamlPreview = () => {
    ...
    const datasetName = formData.name.replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_.-]/g, '').toLowerCase()
    const baseDir = formData.datasetDir || `dataset/${datasetName}`
    
    let yaml = `# Dataset configuration for ${formData.name}
# Generated by AutoTrain Platform

metric: ${formData.format}
num_classes: ${formData.numClasses}

`
    yaml += `TrainDataset:
  name: ${formData.format}DataSet
  image_dir: ${formData.trainImagePath || 'images/train'}
  anno_path: ${formData.trainAnnoPath || 'annotations/train.json'}
  dataset_dir: ${baseDir}
...
`
    yaml += `EvalDataset:
  name: ${formData.format}DataSet
  image_dir: ${formData.evalImagePath || 'images/val'}
  anno_path: ${formData.evalAnnoPath || 'annotations/val.json'}
  dataset_dir: ${baseDir}
  allow_empty: true
                ...
                {generateYamlPreview() && (
                  <div className="col-span-2 space-y-2 pt-4 border-t">
                    <Label className="flex items-center gap-2">
                      ...
                      Generated Dataset YAML Preview
                    </Label>
                    <pre className="p-3 rounded-lg bg-muted/50 text-xs overflow-auto max-h-[200px] font-mono whitespace-pre-wrap">
                      {generateYamlPreview()}
                    </pre>
                  </div>
                )}
